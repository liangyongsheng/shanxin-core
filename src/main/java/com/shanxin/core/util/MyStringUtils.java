...
public abstract class MyStringUtils {
	 ...
	 * 检查指定的字符串是否为空。
	 * <ul>
	 * <li>MyStringUtils.isEmpty(null) = true</li>
	 * <li>MyStringUtils.isEmpty("") = true</li>
	 * <li>MyStringUtils.isEmpty("   ") = true</li>
	 * <li>MyStringUtils.isEmpty("aBc") = false</li>
	 * </ul>
	 * 
	 * @param value
	 *            待检查的字符串
	...
	public static String blur(String input) {
		...
		if (index - 1 > 0)
			rs += input.substring(0, index - 1) + "*";
		if (index + 2 < input.length())
			rs += "**" + input.substring(index + 2, input.length());
		if (rs.equals(""))
			rs = input;
		return rs;
	}
